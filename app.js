     ...
     cartLogic(){
        ...
        cartContent.addEventListener('click', event =>{
            if(event.target.classList.contains('remove-item')){
                ...
                console.log("hie")
                let lowermount = event.target;
                let id = lowermount.dataset.id;
                let tempItem = cart.find(item => item.id===id);
                tempItem.amount = tempItem.amount - 1;
                if(tempItem.amount>0){
                    Storage.saveCart(cart);
                    this.setCartValue(cart);
                    lowermount.previousElementSibling.innerText = tempItem.amount
                }else{
                    cartContent.removeChild(lowermount.parentElement.parentElement)
                    this.removeItem(id)
                }
